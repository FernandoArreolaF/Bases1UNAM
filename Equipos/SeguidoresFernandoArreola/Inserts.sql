...
SELECT agregar_empleado(
    ...
    'https://thispersondoesnotexist.com'::varchar,            	   -- fotografia
...
SELECT agregar_empleado(
    ...
    TRUE,                               -- esMesero
    'Lunes a Viernes de 9 -16 '::varchar,                      -- horario
...
CREATE OR REPLACE FUNCTION agregar_orden(
    ...
    IN p_id_platillos INT[],
    IN p_cantidadesPorProducto SMALLINT[]
)
RETURNS VOID
AS $$
DECLARE
...
BEGIN
    ...
    VALUES (p_folio, CURRENT_TIMESTAMP, p_num_empleado);
	
    -- Iterar sobre los arrays para insertar en la tabla Orden_Platillo
    FOR i IN 1..array_length(p_id_platillos, 1) LOOP
		--Revisar disponibilidad del producto
		IF ((SELECT disponibilidad FROM platillo WHERE(platillo.id_platillo=p_id_platillos[i])) IS NOT FALSE) THEN
			INSERT INTO Orden_Platillo (folio, id_platillo, cantidadPorProducto)
			VALUES (p_folio, p_id_platillos[i], p_cantidadesPorProducto[i]);
		END IF;
		IF ((SELECT disponibilidad FROM platillo WHERE(platillo.id_platillo=p_id_platillos[i])) IS FALSE) THEN
			RAISE NOTICE 'El producto % no se encuentra disponible',p_id_platillos[i];
		END IF;
    END LOOP;

    -- Actualizar el total de la orden a 0; el trigger se encargará del cálculo real
    UPDATE Orden SET total = 0 WHERE folio = p_folio;
END;
$$ LANGUAGE plpgsql;
...
SELECT agregar_orden(
    ...
    1,                   -- num_empleado
    ARRAY[1],                   -- id_platillo
    ARRAY[2::smallint]          -- cantidadPorProducto
);

SELECT agregar_orden(
    'ORD-002'::varchar,  -- folio
    2,                   -- num_empleado
    ARRAY[1,2],                   -- id_platillo
    ARRAY[3::smallint,2::smallint]          -- cantidadPorProducto
);

SELECT agregar_orden(
    'ORD-003'::varchar,  -- folio
    3,                   -- num_empleado
    ARRAY[3],            -- array de id_platillo
    ARRAY[1::smallint]             -- array de cantidadPorProducto
);

SELECT agregar_orden(
    'ORD-004'::varchar,  -- folio
    4,                   -- num_empleado
    ARRAY[4],            -- array de id_platillo
    ARRAY[4::smallint]             -- array de cantidadPorProducto
);

SELECT agregar_orden(
    'ORD-005'::varchar,  -- folio
    5,                   -- num_empleado
    ARRAY[5],            -- array de id_platillo
    ARRAY[2::smallint]             -- array de cantidadPorProducto
);
